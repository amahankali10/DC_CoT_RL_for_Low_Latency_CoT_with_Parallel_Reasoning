...
def get_sharding_strategy(device_mesh, shard_grad_op=False):
...
class ActorRolloutRefWorker(Worker, WorkerProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            ...
            nccl_timeout = self.config.get("nccl_timeout", 7200)
            from datetime import timedelta
            torch.distributed.init_process_group(backend=f"cpu:gloo,{get_device_name()}:{get_nccl_backend()}", rank=rank, world_size=world_size, timeout=timedelta(seconds=nccl_timeout))
    ...
    def _build_model_optimizer(
        ...
        attn_implementation = "sdpa" if self.config.model.get("attn_implementation", "sdpa") == "sdpa" else "flash_attention_2"
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self.config.rollout.name == "vllm_parallel_worker":
                if attn_implementation != "sdpa":
                    raise ValueError(f"attn_implementation {attn_implementation} is not supported for vllm_parallel_worker, only sdpa is supported")
        ...
        shard_grad_op = self.config.model.get("shard_grad_op_only", False)
        sharding_strategy = get_sharding_strategy(fsdp_mesh, shard_grad_op=shard_grad_op)
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        elif rollout_name == "vllm_parallel_worker":
            from verl.workers.rollout.vllm_rollout.vllm_rollout_parallel_workers import vLLMRolloutParallelWorkers
            from verl.workers.sharding_manager.base import BaseShardingManager

            log_gpu_memory_usage(f"Before building {rollout_name} rollout", logger=logger)
            rollout = vLLMRolloutParallelWorkers(
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActor(config=self.config.actor, actor_module=self.actor_module_fsdp, actor_optimizer=self.actor_optimizer, tokenizer=self.tokenizer)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                metrics = self.actor.update_policy(data=data, parallel_workers=(self.config.rollout.name=="vllm_parallel_worker"))
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                output, entropys = self.actor.compute_log_prob(data=data, calculate_entropy=True, parallel_workers=(self.config.rollout.name=="vllm_parallel_worker"))
    ...
    def compute_ref_log_prob(self, data: DataProto):
        if self._is_lora:
            raise NotImplementedError("LoRA not supported right now.")
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output, _ = self.ref_policy.compute_log_prob(data=data, calculate_entropy=False, parallel_workers=(self.config.rollout.name=="vllm_parallel_worker"))
...
class CriticWorker(Worker, WorkerProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self.config.model.use_4d_attn_mask:
                raise NotImplementedError("Did not yet implement critic for parallel workers")
            attn_implementation = "sdpa" if self.config.model.get("attn_implementation", "sdpa") == "sdpa" else "flash_attention_2"
            if self.config.model.use_4d_attn_mask:
                if attn_implementation != "sdpa":
                    raise ValueError("4D attention mask is only supported for sdpa (used with parallel rollout)")
    ...
    def compute_values(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            values = self.critic.compute_values(data=data, parallel_workers=self.config.model.use_4d_attn_mask)
    ...
    def update_critic(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_critic", logger=None) as timer:
                metrics = self.critic.update_critic(data=data, parallel_workers=self.config.model.use_4d_attn_mask)
            delta_time = timer.last
