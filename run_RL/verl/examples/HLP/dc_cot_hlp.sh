#!/bin/bash
#SBATCH --job-name=dc_cot_hlp
#SBATCH --output=dc_cot_hlp.out
#SBATCH -p batch
#SBATCH --nodes=1
...
project_name=${HLP_WANDB_PROJECT}
run_name=${DC_COT_HLP_RUN_NAME}

prev_RL_project=${RL_WANDB_PROJECT}
prev_RL_run_name=${RL_STAGE4_NAME}
prev_RL_step=global_step_220
init_model_path=$main_output_dir/$prev_RL_project/$prev_RL_run_name/$prev_RL_step/hf_format_checkpoint
...
python3 -u -m recipe.dapo.main_dapo \
    ...
    data.gen_batch_size=360 \
    data.max_prompt_length=1024 \
    +data.max_prompt_length_val=2048 \
    data.max_response_length=12000 \
    +actor_rollout_ref.rollout.additional_total_length_limit=21000 \
    +data.seed=105 \
    actor_rollout_ref.model.path=$init_model_path \
    actor_rollout_ref.actor.optim.lr=2e-6 \
    actor_rollout_ref.actor.rl_loss_type=cispo \
    ...
    actor_rollout_ref.actor.logging_prob_ratios_path=$main_output_dir/$project_name/$run_name/logging_prob_ratios \
    ...
    +reward_model.correct_and_no_format_reward=0.01 \
    ...
    algorithm.filter_groups.metric=math_verify_score_list \
    reward_model.overlong_buffer.enable=True \
    +reward_model.overlong_buffer.parallel=True \
    reward_model.overlong_buffer.len=10000 \
    reward_model.overlong_buffer.penalty_factor=0.9 \
    reward_model.overlong_buffer.log=True \
    +reward_model.overlong_buffer.penalize_only_resps_with_correct_and_format=True \
